...
function RoutingControl({ from, to, onRoute, clearRoute }) {
  ...
  useEffect(() => {
    ...
    if (!routingControlRef.current) {
      routingControlRef.current = L.Routing.control({
        ...
        .on('routesfound', function (e) {
          ...
          if (e.routes[0] && e.routes[0].bounds)
    ...
    return () => {
      ...
      if (routingControlRef.current && map) {
        try {
          // 1. Clear waypoints to cancel any ongoing routing
          ...
          // 3. Remove control from map if container exists
  ...
  useEffect(() => {
    ...
    if (from && to) {
      ...
      routingControlRef.current.setWaypoints([L.latLng(from[0], from[1])]);
      if (clearRoute) clearRoute();
    } else {
      routingControlRef.current.setWaypoints([]);
      if (clearRoute) clearRoute();
...
export default function HospitalMap() {
  ...
  const markerRefs = useRef({}); // Store refs to markers by lat,lng

  // Check for URL parameters to pre-select a hospital
  useEffect(() => {
    const lat = searchParams.get('lat');
    const lng = searchParams.get('lng');
    if (lat && lng) {
      setSelectedHospital([parseFloat(lat), parseFloat(lng)]);
    }
  }, [searchParams]);

  // Ensure route is shown when both position and selectedHospital are available
  useEffect(() => {
    const lat = searchParams.get('lat');
    const lng = searchParams.get('lng');
    if (position && lat && lng) {
  ...
  // Automatically open popup for selected hospital
  useEffect(() => {
    if (!selectedHospital) return;
    const key = `${selectedHospital[0]},${selectedHospital[1]}`;
  ...
  useEffect(() => {
    if (!selectedHospital) return;
    const found = hospitals.some(
      (h) => h.lat === selectedHospital[0] && h.lng === selectedHospital[1]
    );
    if (!found) {
    ...
    <div style={{ position: 'relative', height: '100vh' }}>
      ...
      >
        ...
        {hospitals.map((h) => {
          const isSelected =
            selectedHospital &&
            h.lat === selectedHospital[0] &&
            h.lng === selectedHospital[1];
      ...
      {selectedHospital && routeInfo && (
        ...
        >
          <h3 style={{ marginTop: 0, color: '#1976d2', letterSpacing: 1 }}>
            Route Info
          </h3>
          <div style={{ marginBottom: 12, fontWeight: 500 }}>
            <b>Distance:</b>{' '}
            {(routeInfo.summary.totalDistance / 1000).toFixed(2)} km
            <br />
